...
# WARNING: Successful attempts are taken into account too

/ip firewall filter

# If IP is in address list ssh_blacklist, drop the packet.
add chain=input protocol=tcp dst-port=50022 src-address-list=ssh_blacklist \
    action=drop comment="Drop SSH high-frequency attempts" log=yes disabled=no

# If IP in ssh_stage3 and state is new, add IP ssh_blacklist for 1 day.
add chain=input protocol=tcp dst-port=50022 connection-state=new \
    src-address-list=ssh_stage3 action=add-src-to-address-list \
    address-list=ssh_blacklist address-list-timeout=1d \
    comment="Ban IP from SSH for 1 day" log=yes disabled=no
...
add chain=input protocol=tcp dst-port=50022 connection-state=new \
    ...
    comment="Detect 3rd SSH attempt within 2 minutes" log=yes disabled=no
...
add chain=input protocol=tcp dst-port=50022 connection-state=new \
    ...
    comment="Detect 2nd SSH attempt within 1 minute" log=yes disabled=no

# Add IP to address list ssh_stage1.
add chain=input protocol=tcp dst-port=50022 connection-state=new \
    action=add-src-to-address-list address-list=ssh_stage1 \
    address-list-timeout=1m comment="Detect 1st SSH attempt" log=yes disabled=no
